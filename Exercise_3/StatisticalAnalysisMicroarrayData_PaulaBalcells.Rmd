...
# Study "Insights into responses of human astrocytes to H5N1 infection by transcriptional analysis"
...
That is, the control group compared with the infected group at different time points.
...
The limma package extends uses Empirical Bayes models to estimate variability based on both pverall matrix and individuals. The result is stored in a MArrayLM object.
...
topTab_con12vshm12 <- topTable(fit.main, number=nrow(fit.main), coef="con12vshm12", adjust="fdr");
head(topTab_con12vshm12)

topTab_con24vshm24  <- topTable(fit.main, number=nrow(fit.main), coef="con24vshm24", adjust="fdr");
head(topTab_con24vshm24)
```
...
Gene annotation will consist on associating the identifiers on the tables with more common gene names, using the annotation package used for the microarray experiment.
...
```{r}
topAnnotated_con6vshm6 <- annotatedTopTable(topTab_con6vshm6, anotPackage = "hgug4112a.db")
head(topAnnotated_con6vshm6[1:6,1:4])

topAnnotated_con12vshm12 <- annotatedTopTable(topTab_con12vshm12, anotPackage = "hgug4112a.db")
head(topAnnotated_con12vshm12[1:6,1:4])

topAnnotated_con24vshm24 <- annotatedTopTable(topTab_con24vshm24, anotPackage = "hgug4112a.db")
head(topAnnotated_con12vshm12[1:6,1:4])
```
...
We see that most genes are not significant and that only a few ones are down regulated, and even fewer are up regulated.
...
```{r}
listOfTables <- list(con6vshm6 = topTab_con6vshm6, 
                     con12vshm12 = topTab_con12vshm12, 
                     con24vshm24 = topTab_con24vshm24)
listOfSelected <- list()
i = 1
...
```{r}
EntrezUni <- topAnnotated_con24vshm24$ENTREZID
```
...
```{r}
Tab.react <- read.csv2(file.path("./results/ReactomePA.Results.con24vshm24.csv"), sep = ",", header = TRUE, row.names = 1)

Tab.react <- Tab.react[1:4, 1:5]
head(Tab.react)
```

We can conclude that there are differentially expressed genes between the control and the infected astorcyte cells which suggest the over-activated interferon signaling in U251 cells infected by H5N1. This over-activation altogether with an excessive pro-inflammatory response to the infection could contribute to the immunologic injury of the astrocyte cells.